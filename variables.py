###
# code from ethereum-etl-airflow/dags/variables.py ends here
###
...
def read_export_dag_vars(var_prefix, **kwargs):
    ...
    provider_uris_archival = read_var(
        "provider_uris_archival", var_prefix, False, **kwargs
    )
    if provider_uris_archival is None:
        provider_uris_archival = provider_uris
    else:
        provider_uris_archival = [
            uri.strip() for uri in provider_uris_archival.split(",")
        ]
...
def read_parse_dag_vars(var_prefix, dataset, **kwargs):
    ...
    vars = {
        "parse_destination_dataset_project_id": read_var(
            "parse_destination_dataset_project_id", var_prefix, True, **kwargs
        ),
        "schedule_interval": read_var("schedule_interval", var_prefix, True, **kwargs),
        "parse_all_partitions": parse_bool(
            read_var("parse_all_partitions", per_dataset_var_prefix, False),
            default=None,
        ),
        "notification_emails": read_var("notification_emails", None, False, **kwargs),
    }
...
def read_verify_streaming_dag_vars(var_prefix, **kwargs):
    vars = {
        ...
        "verify_partitioned_tables": parse_bool(
            read_var("verify_partitioned_tables", var_prefix, False, **kwargs),
            default=False,
        ),
        ...
        "extra_streaming_tables": read_var(
            "extra_streaming_tables", var_prefix, False, **kwargs
        ),
        "notification_emails": read_var("notification_emails", None, False, **kwargs),
    }
